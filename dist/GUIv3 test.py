...
class App(tk.Tk):
    ...
    def ProductWindow(self):
        ...
        # Labels
        productName = tk.Label(window, text="Product name:")
        productType = tk.Label(window, text="Type of product:")
        dateLabel = tk.Label(window, text='Expiration date:')
        # Inputs and Options
        v = tk.StringVar()
        inputtext = tk.Entry(window, bg='white', textvariable=v)
        listbox = tk.Listbox(window, bg='white', listvariable=var, selectmode=tk.SINGLE)
        ...
        def listbox_command():
            ...
            # Buttons
            buttonNewProduct = tk.Button(window, text="Add product", command=listbox_command, state='disabled')
            if not v.get():
