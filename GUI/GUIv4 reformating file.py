...
class App():
    ...
    def open_window_newProduct(self):
        newProduct(self)
    ...
    def Btn_UI(self):
        ...
        print('button')
        root2 = tk.Toplevel(self._root())
        second = newProduct(root2)
        # Overriding close button
        root2.protocol('WM_DELETE_WINDOW', doSomething)  # root is your root window
        return second
        """
...
class newProduct(): #tk.Toplevel
    def __init__(self, newProd):
        ...
        # newProduct = tk.Toplevel(parent)

        newProd.title('Add Product')
        newProd.focus_force()
        newProd.grab_set()
        ...
        width = 520
        height = 330
        screenwidth = newProd.winfo_screenwidth()
        screenheight = newProd.winfo_screenheight()
        alignstr = '%dx%d+%d+%d' % (width, height, (screenwidth - width) / 2, (screenheight - height) / 2)
        newProd.geometry(alignstr)

        newProd.resizable(width=False, height=False)
        # Labels
        productName = tk.Label(newProd, text="Product name:")
        productType = tk.Label(newProd, text="Type of product:")
        dateLabel = tk.Label(newProd, text='Expiration date:')
        # Inputs and Options
        v = tk.StringVar()
        inputtext = tk.Entry(newProd, bg='white', textvariable=v)
        listbox = tk.Listbox(newProd, bg='white', listvariable=var, selectmode=tk.SINGLE)
        cal = Calendar(newProd, selectmode='day', year=today.year, month=today.month, day=today.day)
        ...
        # Buttons
        buttonNewProduct = tk.Button(newProd, text="Add product", command=listbox_command, state='disabled')
        if not v.get():
